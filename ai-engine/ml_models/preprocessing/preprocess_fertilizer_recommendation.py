...
Purpose : Clean and normalize the fertilizer-recommendation dataset used
          later to train the Fertilizer Recommendation model
          (Random Forest Classifier).

This script ONLY performs cleaning. It does not train any model.

Input  : ai-engine/data/raw/fertilizer_recommendation.csv
Output : ai-engine/data/cleaned/fertilizer_recommendation_cleaned.csv
"""
...
# ── Paths ──────────────────────────────────────────────────────────
RAW_PATH = os.path.join("data", "raw", "fertilizer_recommendation.csv")
CLEANED_DIR = os.path.join("data", "cleaned")
CLEANED_PATH = os.path.join(CLEANED_DIR, "fertilizer_recommendation_cleaned.csv")
...
CATEGORICAL_COLUMNS = [
    ...
    "Irrigation_Type", "Previous_Crop", "Region", "Recommended_Fertilizer",
]
...
def drop_missing_and_duplicates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    (it is a clean, likely synthetically-generated dataset), but this
    guard protects against future data refreshes.
    """
    before = len(df)
...
def normalize_categorical_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize categorical text columns for consistent encoding later:
    - Strip whitespace
    - Title-case crop/soil/region names for uniform display & matching
    Analysis found these columns already well-formed (Clay/Loamy/Sandy/Silt,
    Cotton/Maize/Potato/Rice/Sugarcane/Tomato/Wheat, etc.) but normalization
    is applied defensively for future data batches.
    """
    for col in CATEGORICAL_COLUMNS:
        df[col] = df[col].astype(str).str.strip()
    for col in ["Soil_Type", "Crop_Type", "Previous_Crop", "Crop_Growth_Stage", "Region"]:
        df[col] = df[col].str.title()
    # Season and fertilizer codes (Kharif/Rabi/Zaid, NPK/DAP/MOP/SSP/Urea) are
    # kept as-is since title-casing would break acronyms like "NPK"/"DAP".
    return df
...
def cap_outliers_iqr(df: pd.DataFrame, columns: list) -> pd.DataFrame:
    ...
    all numeric columns in this dataset (synthetic data generated within
    tight, realistic bounds), so this step is a defensive no-op today but
    protects future data pulls.
    """
    for col in columns:
